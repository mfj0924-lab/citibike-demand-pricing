...
from src.pricing.pricing_engine import (
    DynamicPricingEngine, PricingZone, PricingResult,
)
...
class TestDynamicPricingEngine:
    ...
    def test_bike_shortage_surge(self):
        ...
        result = self.engine.predict_price(
            station_id="327", timestamp="2026-06-15 08:00",
            predicted_bike_demand=35, predicted_dock_demand=10,
            available_bikes_now=10, available_docks_now=15,
        )
    ...
    def test_dock_congestion_mild_surge(self):
        ...
        result = self.engine.predict_price(
            station_id="327", timestamp="2026-06-15 18:00",
            predicted_bike_demand=10, predicted_dock_demand=40,
            available_bikes_now=20, available_docks_now=5,
        )
    ...
    def test_balanced_normal(self):
        ...
        result = self.engine.predict_price(
            station_id="327", timestamp="2026-06-15 12:00",
            predicted_bike_demand=10, predicted_dock_demand=10,
            available_bikes_now=20, available_docks_now=20,
        )
        assert result.pricing_multiplier == 1.0
        assert result.suggested_price_usd == 4.49
    ...
    def test_surplus_discount(self):
        ...
        result = self.engine.predict_price(
            station_id="327", timestamp="2026-06-15 03:00",
            predicted_bike_demand=1, predicted_dock_demand=1,
            available_bikes_now=30, available_docks_now=25,
        )
        assert result.pricing_multiplier < 1.0
        assert result.suggested_price_usd < 4.49
    ...
    def test_double_shortage_extreme(self):
        ...
        result = self.engine.predict_price(
            station_id="327", timestamp="2026-06-15 08:00",
            predicted_bike_demand=50, predicted_dock_demand=60,
            available_bikes_now=3, available_docks_now=2,
        )
    ...
    def test_edge_case_zero_bikes(self):
        ...
        result = self.engine.predict_price(
            station_id="327", timestamp="2026-06-15 08:00",
            predicted_bike_demand=5, predicted_dock_demand=3,
            available_bikes_now=0, available_docks_now=10,
        )
        assert result.pricing_multiplier > 1.0
        assert result.suggested_price_usd > 4.49
    ...
    def test_batch_predict(self):
        ...
        results = self.engine.batch_predict(
            ...
            current_bikes=15, current_docks=35,
            total_capacity=50,
        )
...
class TestPricingResult:
    ...
    def test_rounding(self):
        ...
        result = PricingResult(
            station_id="123", timestamp="2026-01-01 00:00",
            predicted_bike_demand=10.0, predicted_dock_demand=8.0,
            available_bikes_now=20, available_docks_now=20,
            bike_gap=10, dock_gap=12,
        ...
        assert len(d.as_tuple().digits) <= 3 if '.' in str(result.suggested_price_usd) else True
